...
3.  {Готово} count++;  сразу об этом. не делай так.  бывают такие обстаятельства когда оно посчитает не правельно.
...
5. {Готово} из выше всего сказаного тогда бы я думаю преобразился бы и  метод добавление студента. например:
	private Student[] members= new Student[10];
...
8. {Готово} метод ту стринг мне не нравиться. тут и нада отдельным методом сдлеать сортировку и в ту стринг тока вывести все и сам фильтр..
...
9.  {Готово} мне не нравиться что ты просто переопределил студента от персоны.  Вроде да в задаче нету такого что в классе студента долзны быть еще свойста. но добавь чтоб было интереснее
...
11. класс персона {Готово}
 ...
 12. исключения  ты просто унаследовался  переопредели хоть 1 метод.  например getMessage {Готово}
 чтоб я  видел что ты умеешь этим всем пользоваться

 Новое задание:
 1. Усовершенствуйте класс Group, добавив возможность
  интерактивного добавления объекта. Имю в виду что с консоли, с  клавиатуры ввод имени, фамилии и т.д.
 2. Тебе там было задача сортировка, фиг с ней как хочешь, листами, интерфейсами. Реализуй возможность сортировки по параметру
 (Фамилия, успеваемость и т. д.). и притом чтоб было кпараметр как  большее к меньшему так и меньшее к большему. Если не понял то напишешь расскажу подробей что хочу
 3. Реализуйте интерфейс Военком, который вернет из группы
  массив студентов - юношей, которым больше 18 лет.

Добавил по задаче
Ну и сортировку пузырьком)
1 .интерактив через метод createGroupeIntaractive, он статический и возращает новую группу заполненую интерактивно
2. Сделал всяких разных сортировок)
3. готово
